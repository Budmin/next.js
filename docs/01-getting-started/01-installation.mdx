...
> **Good to know**: `public` is considered an immutable directory which is read during bootup.
> If the number of files in `public` gets to the hundreds of thousands, you may begin to experience slower startups.
>
> If your application requires a large amount of files, we recommend the use of external file hosts to minimize the amount of files in `public`.
